c1 <- read.table(file="output/cand", sep = "\t", head=TRUE)
c2 <- read.table(file="output/cand", sep = "\t", head=TRUE)
c3 <- read.table(file="output/cand", sep = "\t", head=TRUE)
...
pdf(file="lexinductCAND.pdf", height=6, width=7, onefile=TRUE, family='Helvetica', pointsize=12)
...
title(main="Lexicon Induction Accuracy for so")
