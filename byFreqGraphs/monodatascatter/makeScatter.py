...
for lang in langs:
    ...
    for r in range(1,4):
        ...
        for i, row in enumerate(lines):
            ...
            if i>0:
                ...
                if freq>100:
                    aboverow=i
                    above=freq
                elif freq<=100 and not belowFound:
                    below=freq
        ...
        if aboverow<10:
            ...
            extrapolate=(slope*100)+intercept
            threeestimates.append(extrapolate)
        else:
            threeestimates.append(below)
    top_10_at_f_100.append(sum(threeestimates)/3)
...
outfile.write(','.join([str(x) for x in top_10_at_f_100]))
outfile.write(')\n')

outfile.write('c=plot(monocrawl,top_10_at_f100,log="x",xlab="",ylab="",main="Monolingual Data vs. Lexicon Induction Performance", col="darkgreen",pch=15,cex=0, ylim=c(0,35), xlim=c(0.05,150), xaxt="n", yaxt="n")\n')
...
outfile.write('title(ylab="Accuracy in top-10, word frequency 100",cex.lab=1.5)\n')
