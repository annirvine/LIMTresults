c1 <- read.table(file="run1/aggmrr.table.avgfreq", sep = "\t", head=TRUE)
c2 <- read.table(file="run2/aggmrr.table.avgfreq", sep = "\t", head=TRUE)
c3 <- read.table(file="run3/aggmrr.table.avgfreq", sep = "\t", head=TRUE)
...
title(main="Lexicon Induction Accuracy for Indonesian")
